...
export async function onLeaderboard(
  ...
  const { results } = await env.STATS_DB.prepare(
    ...
    .bind(limit)
    .all<{ uuid: string; username: string; avatar: string; tier: string; value: number }>()

  const toEntry = (r: { uuid: string; username: string; avatar: string; tier: string; value: number }): LeaderboardEntry => ({
    uuid: r.uuid,
    username: r.username,
    avatar: r.avatar,
  ...
  if (UUID_RE.test(uuidParam)) {
    const meRow = await env.STATS_DB.prepare(
      ...
      .bind(uuidParam)
      .first<{ uuid: string; username: string; avatar: string; tier: string; value: number }>()
