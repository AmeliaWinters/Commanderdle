...
/**
 * Length-independent equality that doesn't short-circuit on the first differing byte,
 * so a comparison can't leak where two secrets diverge via timing. (Comparing lengths
 * up front is fine — the length of a secret is not itself sensitive here.)
 */
...
export async function currentUserRow(
  ...
  try {
    ...
    return {
      ...
      avatar: row.avatar,
      tier,
      // Only surface a custom colour while the account still qualifies for it — a
      // lapsed supporter (tier back to 'common') reverts to the default automatically.
